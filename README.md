...
A lending/accounting protocol concept that uses **Bitcoin network difficulty** as an
independent, on-chain, fiat-agnostic signal for BTC demand. Still very much research,
model-first: the repo does the numerical homework and every claim below is backed by a
committed chart under `out/`.
...
> is a proxy for *relative* BTC price, UKNIT-free.**

A protocol loop (RBTC):

1. Deposit BTC/RBTC → mint **SmoothBTC** worth `S(t0) = D_s(t) / D_s(t0)` (D_s = smoothed
   difficulty).
2. If smoother difficulty rises, return SmoothBTC, take back RBTC, re-mint at the higher
...
Because difficulty is a pure protocol-internal measure, SmoothBTC can be *accounted* in
...
that visual correlation is the entire thesis. → `main.py`, `smoothbtc/plot.py` `chart_raw`.
...
1 SmoothBTC, all normalised to a common launch anchor (SMA-350):

- **smoothBTC** — pure difficulty-derived (the real design: `s0 = base · (D/D0)^b`)
- **wma (≈50w)** — SMA-350 of spot, the accounting anchor smoothBTC is meant to track
...
**The headline result:** smoothBTC is dramatically *smooth*. Rolling vol peaks in the
early 2012​–2015 era at ~10–20% for all series, but after ~2018 smoothBTC sits reliably
**under 5% (and often <2%)** while spot and even the 50w/200w SMAs swing 30–80%. SmoothBTC's
...
### Merchant: a USD-priced business accepting SmoothBTC
...
Key figure: **smoothBTC** holds the float's max drawdown at only **−3.2%** vs **−75%** for
spot (the 50w SMA anchor sits between at −54%). A monthly-converted merchant is effectively
neutral-to-mildly-positive, because smoothBTC appreciates while barely ever drawing down
(`05_float_sensitivity.png` shows bigger floats magnify both win and risk).

### Salary: a worker paid fixed SmoothBTC/month

![salary panel](out/backtest/03_salary.png)

A yearly-re-signing salary lands at **~1.2x USDT** — SmoothBTC behaves like a gently-upward,
...
*smooth* `spot/smoothBTC` diverges. Anchored 2016 you need `≈2.9x` backing for "never"; the
...
1. **Mint** — per 1 locked BTC, `mint = (D_ratio)^b / CR`. Today ≈ **48.9 SmoothBTC/BTC**
...
2. **Liquidation** — pure-difficulty floor at `(1/CR)^(1/b) ≈ 20.5%` of mint difficulty:
   ...
   `2020-03-13` spot crash was the worst *spot/smoothBTC* deviation (0.375) → at CR=3 only
...
4. **Prices** — `out/lending_price.png` shows 30-day & 12-month SmoothBTC/USD and the
...
- **SmoothBTC is a stable-ish, difficulty-anchored accounting unit** — ~7× lower vol than
...
smoothbtc/analyze.py     # load data, smoothing windows, OLS fit, metrics
smoothbtc/backtest.py    # value models + cashflow simulators + merchant/collateral metrics
smoothbtc/plot.py        # analysis charts → out/*.png
smoothbtc/download.py     # blockchain.info fetcher, ~1-day cache
