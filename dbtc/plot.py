...
def chart_smoothed_vs_price(df: pd.DataFrame, best_w: int, t0: str) -> Path:
    ...
    ax.set_title(f"normalised log-ratios vs t0={t0}  (smoothing window={best_w} periods)")
    ax.grid(alpha=0.3)
...
def chart_regression(df: pd.DataFrame, best: analyze.WindowResult, t0: str) -> Path:
    ...
    ax.set_title(f"regression ln(price_ratio) ~ ln(diff_ratio)  [window={best.window} periods]")
    ax.legend()
    ax.grid(alpha=0.3)
...
def chart_window_sweep(results: list[analyze.WindowResult]) -> Path:
    ...
    a2.grid(alpha=0.3)
    a2.set_xscale("log")
    a2.set_xlabel("smoothing window (difficulty periods)")
...
def chart_price_space(df: pd.DataFrame, best: analyze.WindowResult, t0: str) -> Path:
    ...
    ax.plot(sub.index, pred_p, lw=1.0, color=COLORS["dbtc"], ls="--",
            label=f"difficulty proxy: price_t0*exp(a+b*ld), W={best.window}p (a={best.intercept:.2f}, b={best.slope:.2f})")
    ax.set_yscale("log")
...
def chart_rolling_corr(df: pd.DataFrame, best_w: int, t0: str, span_days: int = 365) -> Path:
    ...
    ax.set_title(f"rolling correlation of ln(price_ratio) vs ln(diff_ratio), W={best_w} periods")
    ax.grid(alpha=0.3)
